...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        if (window.cmcrpr_listnav_data !== undefined && window.cmcrpr_listnav_data.listnav && window.cmcrpr_listnav_data.list_id) {
            $("#" + window.cmcrpr_listnav_data.list_id).listnav(window.cmcrpr_listnav_data.listnav);
        }
        ...
        function cmcrpr_ajaxSearch (thiz) {
            showLoading();
            var data, currentSearch, previousSearch, searchChanged, tile = thiz.closest('.cmcrpr-item-container');

            previousSearch = tile.find('.cmcrpr-item-search-term').prop('defaultValue') || '';
            currentSearch = tile.find('.cmcrpr-item-search-term').val() || '';
            searchChanged = (previousSearch === currentSearch) ? 0 : 1;

            data = {
                action: 'cmcrpr_search',
                post_id: window.cmcrpr_listnav_data.post_id,
                cat: tile.find('.item-categories').val(),
                freeze_cat: tile.find('.item-freeze-categories').length,
                search_term: currentSearch,
                search_changed: searchChanged,
                isshortcode: tile.find('.item-is-shortcode').val(),
                cmcrpr_title_prefix: tile.find('.cmcrpr_title_prefix').val(),
                cmcrpr_add_category_title: tile.find('.cmcrpr_add_category_title').val(),
                cmcrpr_show_title: tile.find('.cmcrpr_show_title').val(),
                disable_listnav: tile.find('.item-disable-listnav').val(),
                itemspage: getPageNumber(tile.find('.pageNumbers li')),
                letter: getCurrentLetter(tile.find('.ln-letters a')),
                gtags: getSelectedTags(tile.find('.item-tag'))
            };

            $.post(window.cmcrpr_listnav_data.ajaxurl, data, function (response) {
                if (response !== '0') {
                    tile.html($(response));
                }
                hideLoading();
                $('.cmcrpr-item-search-term').select();
                setTimeout(function () {
                    $(document).trigger('cmcrpr_TooltipReady');
                    if (window.cmcrpr_listnav_data !== undefined && window.cmcrpr_listnav_data.listnav && window.cmcrpr_listnav_data.list_id) {
                        $("#" + window.cmcrpr_listnav_data.list_id).listnav(window.cmcrpr_listnav_data.listnav);
                    }
                }, 5);
        ...
        $('.cmcrpr-item-container').on('click', '.cmcrpr-item-search', function () {
            cmcrpr_ajaxSearch($(this));
        });
        $('.cmcrpr-item-container').on('click', '.cmcrpr-item-tag', function () {
            $(this).toggleClass('selected');
            cmcrpr_ajaxSearch($(this));
        });
        $('.cmcrpr-item-container').on('click', '.cmcrpr-item-category', function () {
            $(this).toggleClass('selected');
            var category = $(this).data('category-name');
            $(this).parents('.cmcrpr-item-container').find('.cmcrpr-item-categories').val(category);
            cmcrpr_ajaxSearch($(this));
        });
        $('.cmcrpr-item-container').on('keydown', '.cmcrpr-item-search-term', function (e) {
            if (e.which === 13) {
                e.preventDefault();
                cmcrpr_ajaxSearch($(this));
                return false;
        ...
        $('.cmcrpr-item-container').on('click', '.cmcrpr-item-search-clear', function () {
            var $search;
            $search = $('.cmcrpr-item-search-term');
            $search.val('');
            if ($search.hasClass('search') || $('.cmcrpr-item-tag.selected').length) {
                $('.cmcrpr-item-tag.selected').removeClass('selected');
                cmcrpr_ajaxSearch($(this));
            }
        });
        $('.cmcrpr-item-container').on('keyup', '.cmcrpr-item-search-term', function () {
            var $this, length;
            $this = $(this);
        ...
        $('.cmcrpr-item-container').on('change', '.cmcrpr-item-categories', function () {
            cmcrpr_ajaxSearch($(this));
        });
        $('.cmcrpr-item-container').on('click', 'a.ln-serv-letter', function (e) {

            $(document).find('.cmcrpr-item-container .pageNumbers li').each(function (k, v) {
                var $v = $(v);
                if ($v.hasClass('selected'))
                {
            ...
            $(document).find('.cmcrpr-item-container a.ln-serv-letter').each(function (k, v) {
                var $v = $(v);
                if ($v.hasClass('ln-selected'))
                {
        ...
        $(document).on('mouseover', '.cmcrpr-item-container a.ln-serv-letter', function (e) {
        ...
        $(document).on('mouseout', '.cmcrpr-item-container a.ln-serv-letter', function () {
            $('div.ln-letter-count').hide();
        });

        $(document).on('click', '.cmcrpr-item-container .pageNumbers a', function (e) {
            $(document).find('.cmcrpr-item-container .pageNumbers li').each(function (k, v) {
                var $v = $(v);
                if ($v.hasClass('selected'))
                {
